...
import { PlusIcon, ButtonBg, LiveBetBgMobile, ArrowIcon } from '@/assets/svg';
...
const MOCK_COMPLETED_BETS: BetData = [
...
const History: React.FC<CompletedBetsProps> = ({ setCompletedBetsCount }) => {
  ...
  useEffect(() => {
    setCompletedBetsData(MOCK_COMPLETED_BETS);
    setCompletedBetsCount(MOCK_COMPLETED_BETS.length);
  }, [setCompletedBetsCount]);
      ...
      <div className="livebets">
        {completedBetsData.length === 0 ? (
          ...
          completedBetsData.map((bet) => (
            <div className="livebet" key={bet._betId}>
